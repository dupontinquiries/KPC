...
public class AquaParkour extends ParkourBase {
    ...
    public AquaParkour(final double universalMultiplier, double a, double s) {
        super(universalMultiplier); //, a, s);
        setup();
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World e, PlayerEntity player, Hand handIn) {
        ...
        if (ready == 0) {
            ...
            if (!player.getCooldownTracker().hasCooldown(this) && player.isInWater() && !player.abilities.isFlying) {
    ...
    protected void leapTriggered(World world, PlayerEntity player, Hand hand) {
        ...
        world.playSound(null, player.getPosition(), leapSound.getSound(), SoundCategory.AMBIENT, 1F, 1F + ( Item.random.nextFloat() * 0.4F ) - 0.2F );
