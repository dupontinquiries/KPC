...
public class SwirlFireBall extends SmallFireballEntity {
    ...
    public void tick() {
        ...
        if ( newTicks % 5 == 0 ) {
            for ( Coord c : Shapes.makeSphere(world, getPosition(), 1, true) ) {
                for ( char a = 0; a < world.rand.nextInt(6) + 3; ++a ) {
                    world.addOptionalParticle(ParticleTypes.SMOKE, c.getX() + offset(world.getRandom()), c.getY() + offset(world.getRandom()), c.getZ() + offset(world.getRandom()),
