package media.kitchen.kitchenparkour.itemtype.parkour;
...
public class ParkourBase extends SwordBase {
    ...
    public ParkourBase(Item.Properties props, final double xz, final double y) {
        this(new Item.Properties().group(ItemGroup.COMBAT).maxStackSize(1).defaultMaxDamage(550), (double) ((xz + y)/2), true);
        multiplierXZ = xz;
        multiplierY = y;
    }

    public ParkourBase(Item.Properties props, final double universalMultiplier) {
        this(new Item.Properties().group(ItemGroup.COMBAT).maxStackSize(1).defaultMaxDamage(550), universalMultiplier, true);
        multiplierXZ = multiplierY = universalMultiplier;
    }

    public ParkourBase(final double xz, final double y, int a, float s) {
        this(new Item.Properties().group(ItemGroup.COMBAT).maxStackSize(1).defaultMaxDamage(550), 1D, true);
        multiplierXZ = xz;
        multiplierY = y;
        //setAttackAndSpeed(a, s);
    }
    ...
    public ParkourBase(final double universalMultiplier, int a, float s) {
        this(new Item.Properties().group(ItemGroup.COMBAT).maxStackSize(1).defaultMaxDamage(550), 1D, true);
        multiplierXZ = multiplierY = universalMultiplier;
    }
    ...
    public ParkourBase(Item.Properties props, double extra, boolean main) {
        super(ItemTier.DIAMOND, (int) (4.6 * (extra)), 0.8F, props);
    ...
    */
    ...
    public void onCreated(ItemStack stack, World worldIn, PlayerEntity playerIn) {
        worldIn.playSound(null, playerIn.getPosition(), Parkour.PARKOUR_GRIPPER_READY.get(), SoundCategory.AMBIENT, 1F, 1F + ( random.nextFloat() * 0.4F ) - 0.2F );
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World e, PlayerEntity player, Hand handIn) {
        ...
        if (ready == 0) {
            ...
            if (!player.getCooldownTracker().hasCooldown(this) /*cooldown == 0*/ && uses > 0
                    && !(player.isInWater() || player.abilities.isFlying || player.isOnGround())) {
    ...
    protected void leapTriggered(World world, PlayerEntity player, Hand hand) {
        Vector3d path = Vector3d.fromPitchYaw(player.rotationPitch, player.rotationYaw);
        ...
        world.playSound(null, player.getPosition(), leapSound.getSound(), SoundCategory.AMBIENT, 1F, 1F + ( random.nextFloat() * 0.4F ) - 0.2F );
    ...
    protected void wallLeapTriggered(World world, PlayerEntity player, Hand hand) {
        ...
        world.playSound(null, player.getPosition(), leapSound.getSound(), SoundCategory.AMBIENT, 1F, 1F);
    }
    ...
    public void inventoryTick(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (!isSelected) {
            ...
            if (ready > 0) {
                if (ready == 1) {
                    worldIn.playSound(null, entityIn.getPosition(), Parkour.PARKOUR_GRIPPER_READY.get(), SoundCategory.AMBIENT, 1F, 1F + ( random.nextFloat() * 0.4F ) - 0.2F );
                }
                --ready;
            } else {
                // !code for tracking timers
                if (entityIn instanceof PlayerEntity) {
                    ...
                    if ( cooldown > 0 & (!player.isAirBorne || player.isOnLadder()) ) { ////// will be obsolete
                        --cooldown;
                    }
                    if (wallLeap > 0) {
                        --wallLeap;
                    ...
                    if (glide > 0 && player.isAirBorne && !worldIn.getBlockState(new BlockPos(i, j, k)).isSolid()) {
                        ...
                        if (wallLeap > 0) {
                            Vector3d path = Vector3d.fromPitchYaw(player.rotationPitch, player.rotationYaw); // player.getForward().normalize();
    ...
    public boolean getIsRepairable(ItemStack toRepair, ItemStack repair) {
        return repair.getItem() == Parkour.RUBY_POWDER.get();
    }
}
