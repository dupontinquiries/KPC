...
public class SuperTrident extends TridentItem {
    public SuperTrident(Properties builder) {
        super(builder);
    }
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerentity.isWet()) {
                    if (!worldIn.isRemote) {
                        ...
                        if (j == 0) {
                            SuperTridentEntity proj = new SuperTridentEntity(worldIn, playerentity, stack);
                            proj.shoot(playerentity, playerentity.rotationPitch, playerentity.rotationYaw, 0.0F, 2.5F + (float)j * 0.5F, 1.0F);
                            if (playerentity.abilities.isCreativeMode) {
                                proj.pickupStatus = AbstractArrowEntity.PickupStatus.CREATIVE_ONLY;
                            }

                            worldIn.addEntity(proj);
                            worldIn.playMovingSound((PlayerEntity)null, proj, SoundEvents.ITEM_TRIDENT_THROW, SoundCategory.PLAYERS, 1.0F, 1.0F);
                            if (!playerentity.abilities.isCreativeMode) {
