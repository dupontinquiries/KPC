...
public class SolarBoots extends SolarBase implements IHasModel {

	public SolarBoots(String name, ArmorMaterial materialIn, int renderIndexIn, EntityEquipmentSlot equipmentSlotIn) {
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack stack)
    {
		...
		if (worldIn.provider.hasSkyLight())
        {
            ...
            if (i > 3 && !player.capabilities.isCreativeMode) {
            	if (!player.isInWater() && !player.capabilities.isFlying && !player.isOnLadder() && player.isPotionActive(Potion.getPotionFromResourceLocation("strength"))) {
